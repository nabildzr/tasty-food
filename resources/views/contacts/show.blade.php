...
@section('title', isset($result) ? 'Edit User' : 'Create User')
        ...
        <div class="container-fluid">
            <div class="page-header-content">
                <h1 class="page-header-title">
                    ...
                    <span>{{ isset($result) ? 'Edit User' : 'Create User' }}</span>
                </h1>
                ...
                <div id="default">
                    <div class="card mb-4">
                        <div class="card-header">
                            {{ isset($result) ? "Edit $result->name User" : 'Create new User' }}
                        </div>
                                            ...
                                            <select name="role_id" class="form-control " id="role">
                                                ...
                                                @foreach ($roles as $role)
                                                    <option value="{{ $role->id }}"
                                                        {{ old('role_id', isset($result) ? $result->role_id : '') == $role->id ? 'selected' : '' }}>
                                                        {{ $role->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                        ...
                                        @if (!isset($result))
                                            <div class="form-group">
                                                ...
                                                <input class="form-control" id="password" name="password" type="text"
                                                    value="{{ old('password', isset($result) ? $result->password : '') }}"
